...
public class Main
{
    public static void main(String[] args)
    {
        ...
        //производим поиск перебирая итератором объекты. Если параметр объекта равен или больше, вкупе с остальными параметрами, то подходит.
        //Если параметр не указан, то -1 выступает как заглушка.

        int counter = 0;
        Iterator<Laptop> iteratorLaptopss = Laptopss.iterator();
        while (iteratorLaptopss.hasNext())
        //for (Laptop thisLaptop : Laptopss)
        {
            counter++;
            Laptop thisLaptop = iteratorLaptopss.next();
            if (
                    (thisLaptop.getValueRAM() >= Integer.parseInt(quaryList.get(0)) || Integer.parseInt(quaryList.get(0)) == -1) &&
                            (thisLaptop.getValueHDD() >= Integer.parseInt(quaryList.get(1)) || Integer.parseInt(quaryList.get(1)) == -1) &&
                            (thisLaptop.getSizeIches() >= Integer.parseInt(quaryList.get(2)) || Integer.parseInt(quaryList.get(2)) == -1) &&
                            (thisLaptop.getFrequencyCPU() >= Double.parseDouble(quaryList.get(3)) || Double.parseDouble(quaryList.get(3)) == -1) &&
                            (thisLaptop.getPrice() >= Double.parseDouble(quaryList.get(4)) || Integer.parseInt(quaryList.get(4)) == -1)
